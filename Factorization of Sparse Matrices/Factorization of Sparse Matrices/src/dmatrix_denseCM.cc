...
dmatrix_denseCM generateRandom(int m, int n, double min, double max){
  ...
  std::uniform_real_distribution<> dis(min, max); // this is to have an uniform distribution between 0. and 1.
...
int  computeBandwidthDown(const dmatrix_denseCM &A ){
  ...
  for (int i = 0; i < m; ++i){
    for (int k = 1; k< m-i; ++k){
      if (A(i+k,i) != 0.0) {lbdown = std::max(k,lbdown);}
    }
  }
  return lbdown;
};
...
dmatrix_denseCM sympermut(const dmatrix_denseCM &A, const std::vector<int > &p){
  int m = A.getNbLines();
  assert((m == A.getNbColumns()) && (m== p.size()));
  dmatrix_denseCM B(m,m);
